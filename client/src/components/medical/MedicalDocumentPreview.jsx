...
export function MedicalDocumentPreview({ document, onBack }) {
  ...
  const rawText = document.extractedData?.rawText
    ?? document.extractedData?.raw_text
    ?? document.ocr?.rawText
    ?? "";
  const normalizedRawText = String(rawText).replace(
    /\b(?:upload-)?[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}\b/gi,
    ""
  ).trim();
  ...
  return (
    <div className="flex h-full w-full min-w-0 flex-1 min-h-0 flex-col">
      ...
      <div className={`flex w-full min-w-0 min-h-0 flex-1 gap-4 overflow-hidden ${hasAnalysis ? 'flex-col lg:flex-row' : 'flex-col'}`}>
        <div className="flex-1 min-w-0 min-h-0 overflow-hidden rounded-xl border border-[#d1e2dc] bg-[#f7fbf9]">
          {isPdf ? (
        ...
        {document.analysisStatus === "completed" && (
          <div className="flex flex-1 min-w-0 min-h-0 flex-col gap-5 overflow-hidden rounded-xl border border-[#d1e2dc] bg-white p-5 shadow-sm">
            <div className="border-b border-[#d1e2dc] pb-3">
              <h3 className="font-bold text-[#0c5e5b] text-lg">OCR Raw Text</h3>
            ...
            <div className="flex-1 min-w-0 min-h-0 overflow-x-hidden overflow-y-auto whitespace-pre-wrap break-words rounded-xl border border-[#d1e2dc] bg-[#f4f9f7] p-4 text-sm leading-relaxed text-[#142a30]">
              {normalizedRawText || "No raw OCR text is available for this document."}
            </div>
          </div>
        )}
      </div>
